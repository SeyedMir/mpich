...
int MPIR_Neighbor_allgather_init_impl(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                      void *recvbuf, int recvcount, MPI_Datatype recvtype,
                                      MPIR_Comm *comm_ptr, MPI_Request *request)
{
    ...
    if(topo_ptr->topo.dist_graph.shm_nbh_coll_sched != NULL)
    {
        ...
        if(topo_ptr->topo.dist_graph.sched_mem_to_free[0])
...
#ifdef HAVE_ERROR_CHECKING
    {
        mpi_errno =
            MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, FCNAME, __LINE__, MPI_ERR_OTHER,
                                 ...
                                 "**mpi_neighbor_allgather_init %p %d %D %p %d %D %C %p", sendbuf,
