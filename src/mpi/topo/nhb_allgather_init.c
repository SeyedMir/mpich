...
//SHM
#undef FUNCNAME
#define FUNCNAME MPIR_Neighbor_allgather_init_impl
#undef FCNAME
...
int MPIR_Neighbor_allgather_init_impl(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPIR_Comm *comm_ptr, MPI_Request *request)
{
    ...
    {
        fprintf(stderr, "ERROR: Topology is not MPI_DIST_GRAPH; Aborting MPI_Neighbor_allgather_start\n");
        goto fn_fail;
    }
    ...
    if(nbr_impl == 1) //Use SHM implementation -- This is not the best place for this statement
        mpi_errno = MPIR_Ineighbor_allgather_SHM(sendbuf, sendcount, sendtype,
        										 ...
        										 comm_ptr, s, 1); //1 to denote a persistent call
	else
...
#undef FUNCNAME
#define FUNCNAME MPI_Neighbor_allgather_init
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
/*@
MPI_Neighbor_allgather_init - Persistent version of MPI_Neighbor_allgather.
...
.N Errors
@*/
int MPI_Neighbor_allgather_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPI_Comm comm, MPI_Request *request)
{
    int mpi_errno = MPI_SUCCESS;
    MPIR_Comm *comm_ptr = NULL;
    MPIR_FUNC_TERSE_STATE_DECL(MPID_STATE_MPI_NEIGHBOR_ALLGATHER_INIT);

    MPID_THREAD_CS_ENTER(GLOBAL, MPIR_THREAD_GLOBAL_ALLFUNC_MUTEX);
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_INEIGHBOR_ALLGATHER_INIT);
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    mpi_errno = MPIR_Neighbor_allgather_init_impl(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, request);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);

    /* ... end of body of routine ... */

fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_INEIGHBOR_ALLGATHER_INIT);
...
#   ifdef HAVE_ERROR_CHECKING
    {
        mpi_errno = MPIR_Err_create_code(
            ...
            "**mpi_neighbor_allgather_init", "**mpi_neighbor_allgather_init %p %d %D %p %d %D %C %p", sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, request);
    }
#   endif
