...
int MPI_Neighbor_allgather_start(MPI_Comm comm, MPI_Request *request)
{
     ...
     * communicator. I'm doingit this way because I'm
    ...
    {
        fprintf(stderr, "ERROR: shm_nbh_coll_sched is NULL (not built yet); Aborting MPI_Neighbor_allgather_start\n");
        goto fn_fail;
    }
    ...
    *request = MPI_REQUEST_NULL;
    ...
    if(comm_ptr->rank == 0)
        printf("Time to clone the schedule in MPI_Neighbor_allgather_start (rank0): %lf   (s)\n", sched_clone_time);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    */
